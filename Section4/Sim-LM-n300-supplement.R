...
## results box
meth <- c("RM1", "RM2","sRM1", "aRM2", "aRM3", "T", "MT")
L <- length(meth)
...
for(j in 1:J){
  ...
  for(r in 1:R){
    ...
    CI.Beta <- list()
    for(l in 1:L){
      if(l==1){ fit <- EHE(Y, X, gam.est=F, mc=mc, burn=bn, gam=0.5) }
      if(l==2){ fit <- EHE(Y, X, gam.est=F, mc=mc, burn=bn, gam=0.2) }
      if(l==3){ fit <- EHE(Y, X, gam.est=T, mc=mc, burn=bn, gam.hp=c(10,100)) }
      if(l==4){ fit <- EHE(Y, X, gam.est=T, mc=mc, burn=bn, gam.hp=c(1,1)) }
      if(l==5){ fit <- EHE(Y, X, gam.est=T, mc=mc, burn=bn, gam.hp=c(10,10)) }
      if(l==6){ fit <- TR(Y, X, mc=mc, burn=bn, nu=2.1) }
      if(l==7){ fit <- MT(Y, X, mc=mc, burn=bn, nu=2.1) }
...
## save the result
save(list=ls(), file=paste0("sim-sub-a", aa, ".RData"))
