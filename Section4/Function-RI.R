...
###  LPM distribution  ###
EHE.RI <- function(Y, X, ID, Gam=1, gam.est=F, mc=2000, burn=500){
  ...
  for(k in 1:mc){
    ...
    if(gam.est){
      gam.delta <- 100
      Gam <- rgamma(1, gam.delta/10+N, gam.delta+sum(log(1+log(1+U2))) )
      Gam.pos[k] <- Gam
    }
  ...
  Res <- list(Beta=Beta.pos, RE=RE.pos, Tau=Tau.pos, Sig=Sig.pos, Om=om.pos, Gam=Gam.pos)
  return(Res)
}
...
###  t-distribution (two-component mixture)   ###
mix.tBR.RI <- function(Y, X, ID, nu=0.5, mc=2000, burn=500){
  delta <- 1   # hyperparameter for sigma
  XX <- cbind(1, X)
  m <- length(unique(ID))
  n <- sum(ID==1)
...
###  normal distribution   ###
BR.RI <- function(Y, X, ID, mc=2000, burn=500){
  m <- length(unique(ID))
  n <- sum(ID==1)
  N <- length(Y)
  Int.pos <- c()
